...
public class Exam03 {
  ...
  static void test(A3 obj) {
    ...
    //
    // 답변:
    // - A3은 추상 클래스이기 때문에 A3의 인스턴스는 생성할 수 없다.
    // - 따라서 obj 파라미터에 넘어오는 인스턴스(주소)는 
    //   A3의 인스턴스가 아니라 A3의 자식 클래스의 인스턴스일 것이다.
    // - 인스턴스를 만들었다는 의미는 A3의 자식 클래스로서
    //   A3의 모든 추상 메서드를 구현했다는 것이다.
    // - 따라서 obj를 가지고 m1을 호출할 때는
    //   실제 obj가 가리키는 인스턴스의 클래스에서 m1() 메소드를 찾아 호출한다.
    // - 다시 한 번 말하지만 인스턴스를 만들었다는 것은 일반 클래스라는 뜻이고,
    //   일반 클래스에는 추상 메서드가 없다. 그러므로, A3를 상속받은 일반 클래스는 
    //   반드시 m1() 메서드를 구현했다는 의미이다.
  }
}
